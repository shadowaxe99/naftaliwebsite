...
export default function LockScreen({ onAuthenticate, isDarkMode, toggleTheme }: { onAuthenticate: () => void, isDarkMode: boolean, toggleTheme: () => void }) {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-6 relative overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-[#171717]' : 'bg-neutral-100'}`}>
      ...
      <div className={`absolute top-10 left-10 text-[10px] font-mono uppercase tracking-[0.3em] hidden lg:block select-none ${isDarkMode ? 'text-white/20' : 'text-black/20'}`}>
      ...
      >
        <div className="flex justify-center mb-8">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center border shadow-inner ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-black/5 border-black/10'}`}>
            <Lock className={isDarkMode ? 'text-white/80' : 'text-black/80'} size={28} />
          </div>
        </div>
